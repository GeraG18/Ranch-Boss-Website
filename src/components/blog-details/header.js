...
function BlogHeader ({author="", authorImg = "", title="", tags=[], image="", date="", body=""}){
    ...
    return (
        ...
        bg-no-repeat bg-center bg-cover overflow-hidden font-['Montserrat'] h-[80vh] lg:h-[70vh]">
            ...
            mx-4 max-w-(--breakpoint-xl) z-100 xl:mx-auto">
                ...
                text-shadow flex-col">
                    <span className="max-w-[95%] h-auto font-['Michroma'] text-[3rem] leading-[2.8rem] 
                    uppercase text-left text-white z-20">{title || '-'}</span>
                    <div className="relative z-30 w-full py-2 flex flex-row flex-wrap gap-1">
                        {
                            tags.map((tag, index) => (
                                <div key={index} className="w-fit rounded-[10px] py-0.5 px-3 uppercase
