            ...
            onClick={(e) => e.stopPropagation()}>
                ...
                <div className="p-4 flex flex-col gap-4 justify-center items-center flex-wrap overflow-x-hidden overflow-y-auto h-fit">
                    ...
                    <div className="flex flex-row gap-2 items-center justify-center w-full">
                        <button className="col-span-full row-start-3 font-['Montserrat'] cursor-pointer relative text-white border-none bg-[#7a7a7a] text-[1rem] select-none 
                    motion-safe:transition-all motion-reduce:transition-none will-change-auto py-3 px-10 w-full rounded-[10px] lg:hover:bg-gradient-to-b lg:hover:from-secondary-color lg:hover:to-primary-color font-medium lg:hover:text-white self-center justify-self-center" 
                            onClick={() => onDialogChanges(false)}>Cancel</button>
                        <button className="col-span-full row-start-3 font-['Montserrat'] cursor-pointer relative text-white border-none bg-primary-color text-[1rem] select-none 
                    motion-safe:transition-all motion-reduce:transition-none will-change-auto py-3 px-10 w-full rounded-[10px] lg:hover:bg-gradient-to-b lg:hover:from-secondary-color lg:hover:to-primary-color font-medium lg:hover:text-white self-center justify-self-center" onClick={() => onDialogChanges(true)}>Accept</button>
                    </div>
                </div>
            </div>
        </div>
